...
pub struct GroupRowQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
...
where
    C: GenericClient,
{
    pub fn map<R>(self, mapper: fn(GroupRowBorrowed) -> R) -> GroupRowQuery<'c, 'a, 's, C, R, N> {
        GroupRowQuery {
            client: self.client,
            params: self.params,
            stmt: self.stmt,
...
pub struct GroupSummaryRowQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
    params: [&'a (dyn postgres_types::ToSql + Sync); N],
    stmt: &'s mut crate::client::async_::Stmt,
    ...
    C: GenericClient,
{
    ...
    ) -> GroupSummaryRowQuery<'c, 'a, 's, C, R, N> {
...
pub fn select_groups() -> SelectGroupsStmt {
    SelectGroupsStmt(crate::client::async_::Stmt::new(
        "SELECT \"group\".*, coalesce(\"users\".json_data, '[]'::json) AS \"users\", coalesce(collections.json_data, '[]'::json) AS collections, coalesce(pricelists.json_data, '[]'::json) AS price_lists FROM \"group\" LEFT JOIN LATERAL ( SELECT group_user.group_id, json_agg(json_build_object( 'id', \"user\".id, 'name', \"user\".name, 'email', \"user\".email, 'last_sign_in', \"user\".last_sign_in, 'profile_image', \"user\".profile_image )) FILTER (WHERE \"user\".id IS NOT NULL) AS json_data FROM group_user INNER JOIN \"user\" ON \"user\".id = group_user.user_id WHERE group_user.group_id = \"group\".id GROUP BY group_user.group_id ) AS \"users\" ON \"users\".group_id = \"group\".id LEFT JOIN LATERAL ( SELECT group_collection.group_id, json_agg( json_build_object( 'id', collection.id, 'acronym', collection.acronym, 'name', collection.name, 'image_url', collection.image_url, 'external_id', collection.external_id, 'slug', collection.slug, 'created_by', collection.created_by, 'created_at', collection.created_at, 'updated_at', collection.updated_at, 'pricing', coalesce(pricing.json_data, '[]'::json), 'num_styles', coalesce(stats.num_styles, 0), 'num_colors', coalesce(stats.num_colors, 0), 'num_sizes', coalesce(stats.num_sizes, 0) ) ORDER BY collection.id DESC ) FILTER (WHERE collection.id IS NOT NULL) AS json_data FROM group_collection INNER JOIN collection ON collection.id = group_collection.collection_id LEFT JOIN ( SELECT size_collection.collection_id, count(DISTINCT size.id) AS num_sizes, count(DISTINCT color.id) AS num_colors, count(DISTINCT color.style_id) AS num_styles FROM size_collection INNER JOIN size ON size.id = size_collection.size_id INNER JOIN color ON size.color_id = color.id GROUP BY size_collection.collection_id ) AS stats ON stats.collection_id = group_collection.collection_id LEFT JOIN LATERAL ( SELECT collection_pricelist.collection_id, json_agg( json_build_object( 'list', json_build_object( 'id', pricelist.id, 'external_id', pricelist.external_id, 'slug', pricelist.slug, 'name', pricelist.name ), 'date', collection_pricelist.price_date ) ) FILTER (WHERE collection_pricelist.collection_id IS NOT NULL) AS json_data FROM collection_pricelist INNER JOIN pricelist ON pricelist.id = collection_pricelist.pricelist_id WHERE collection_pricelist.collection_id = collection.id GROUP BY collection_pricelist.collection_id ) AS pricing ON pricing.collection_id = collection.id WHERE group_collection.group_id = \"group\".id GROUP BY group_collection.group_id ) AS collections ON collections.group_id = \"group\".id LEFT JOIN LATERAL ( SELECT group_pricelist.group_id, json_agg( json_build_object( 'id', pricelist.id, 'name', pricelist.name, 'slug', pricelist.slug, 'external_id', pricelist.external_id ) ) FILTER (WHERE pricelist.id IS NOT NULL) AS json_data FROM group_pricelist INNER JOIN pricelist ON pricelist.id = group_pricelist.pricelist_id WHERE group_pricelist.group_id = \"group\".id GROUP BY group_pricelist.group_id ) AS pricelists ON pricelists.group_id = \"group\".id WHERE \"group\".organization_id = $1 AND ($2::int IS NULL OR \"group\".id = $2) AND ($3::text IS NULL OR \"group\".external_id = $3) AND ($4::text IS NULL OR \"group\".slug = $4) ORDER BY \"group\".updated_at DESC",
    ))
}
...
impl SelectGroupsStmt {
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::StringSql, T2: crate::StringSql>(
        ...
        slug: &'a Option<T2>,
    ) -> GroupRowQuery<'c, 'a, 's, C, GroupRow, 4> {
        GroupRowQuery {
            client,
...
pub fn select_group_summaries() -> SelectGroupSummariesStmt {
    SelectGroupSummariesStmt(crate::client::async_::Stmt::new(
        "SELECT \"group\".id, \"group\".slug, \"group\".external_id, \"group\".name, \"group\".description, coalesce(user_counts.n, 0) AS num_users, coalesce(collection_counts.n, 0) AS num_collections, coalesce(pricelist_counts.n, 0) AS num_price_lists FROM \"group\" LEFT JOIN LATERAL ( SELECT group_user.group_id, count(*) AS n FROM group_user WHERE group_user.group_id = \"group\".id GROUP BY group_user.group_id ) AS user_counts ON user_counts.group_id = \"group\".id LEFT JOIN LATERAL ( SELECT group_collection.group_id, count(*) AS n FROM group_collection WHERE group_collection.group_id = \"group\".id GROUP BY group_collection.group_id ) AS collection_counts ON collection_counts.group_id = \"group\".id LEFT JOIN LATERAL ( SELECT group_pricelist.group_id, count(*) AS n FROM group_pricelist WHERE group_pricelist.group_id = \"group\".id GROUP BY group_pricelist.group_id ) AS pricelist_counts ON pricelist_counts.group_id = \"group\".id WHERE \"group\".organization_id = $1 AND ($2::int IS NULL OR \"group\".id = $2) AND ($3::text IS NULL OR \"group\".external_id = $3) AND ($4::text IS NULL OR \"group\".slug = $4) ORDER BY \"group\".updated_at DESC",
    ))
}
...
impl SelectGroupSummariesStmt {
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::StringSql, T2: crate::StringSql>(
        ...
        external_id: &'a Option<T1>,
        slug: &'a Option<T2>,
    ) -> GroupSummaryRowQuery<'c, 'a, 's, C, GroupSummaryRow, 4> {
        GroupSummaryRowQuery {
            client,
            params: [organization_id, id, external_id, slug],
            stmt: &mut self.0,
...
pub fn get_group_id() -> GetGroupIdStmt {
    GetGroupIdStmt(crate::client::async_::Stmt::new(
        "SELECT \"group\".id FROM \"group\" WHERE \"group\".organization_id = $1 AND ($2::int IS NULL OR \"group\".id = $2) AND ($3::text IS NULL OR \"group\".external_id = $3) AND ($4::text IS NULL OR \"group\".slug = $4)",
    ))
}
...
pub fn insert_group() -> InsertGroupStmt {
    InsertGroupStmt(crate::client::async_::Stmt::new(
        "INSERT INTO \"group\" ( slug, external_id, name, description, organization_id, created_by) VALUES ( $1, $2, $3, $4, $5, $6) RETURNING id",
    ))
}
...
pub fn update_group() -> UpdateGroupStmt {
    UpdateGroupStmt(crate::client::async_::Stmt::new(
        "UPDATE \"group\" SET slug = coalesce($1, slug), external_id = coalesce($2, external_id), name = coalesce($3, name), description = coalesce($4, description) WHERE id = $5",
    ))
}
...
pub fn delete_group() -> DeleteGroupStmt {
    DeleteGroupStmt(crate::client::async_::Stmt::new(
        "DELETE FROM \"group\" WHERE organization_id = $1 AND id = $2",
    ))
}
...
pub fn replace_group_users() -> ReplaceGroupUsersStmt {
    ReplaceGroupUsersStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM replace_group_users($1, $2)",
    ))
}
...
pub fn replace_group_collections() -> ReplaceGroupCollectionsStmt {
    ReplaceGroupCollectionsStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM replace_group_collections($1, $2)",
    ))
}
...
pub fn replace_group_pricelists() -> ReplaceGroupPricelistsStmt {
    ReplaceGroupPricelistsStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM replace_group_pricelists($1, $2)",
    ))
}
