...
pub struct CollectionRowQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
    ...
    C: GenericClient,
{
    ...
    ) -> CollectionRowQuery<'c, 'a, 's, C, R, N> {
        CollectionRowQuery {
            client: self.client,
            params: self.params,
            stmt: self.stmt,
...
pub struct CollectionSummaryRowQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
    params: [&'a (dyn postgres_types::ToSql + Sync); N],
    stmt: &'s mut crate::client::async_::Stmt,
    ...
    C: GenericClient,
{
    ...
    ) -> CollectionSummaryRowQuery<'c, 'a, 's, C, R, N> {
...
pub fn select_collections() -> SelectCollectionsStmt {
    SelectCollectionsStmt(crate::client::async_::Stmt::new(
        "SELECT collection.*, coalesce(pricing.json_data, '[]'::json) AS pricing, coalesce(sizes.json_data, '[]'::json) AS sizes FROM collection INNER JOIN ( SELECT group_collection.collection_id FROM group_collection INNER JOIN group_user ON group_user.group_id = group_collection.group_id WHERE group_user.user_id = $1 GROUP BY group_collection.collection_id UNION SELECT collection.id AS collection_id FROM collection WHERE collection.created_by = $1 ) AS requester_collections ON requester_collections.collection_id = collection.id LEFT JOIN ( SELECT size_collection.collection_id, json_agg( size.* ORDER BY size_collection.position ) FILTER (WHERE size.id IS NOT NULL) AS json_data FROM size INNER JOIN size_collection ON size_collection.size_id = size.id GROUP BY size_collection.collection_id ) AS sizes ON sizes.collection_id = collection.id LEFT JOIN LATERAL ( SELECT collection_pricelist.collection_id, json_agg( json_build_object( 'list', json_build_object( 'id', pricelist.id, 'external_id', pricelist.external_id, 'slug', pricelist.slug, 'name', pricelist.name ), 'date', collection_pricelist.price_date ) ) FILTER (WHERE collection_pricelist.collection_id IS NOT NULL) AS json_data FROM collection_pricelist INNER JOIN pricelist ON pricelist.id = collection_pricelist.pricelist_id WHERE collection_pricelist.collection_id = collection.id GROUP BY collection_pricelist.collection_id ) AS pricing ON pricing.collection_id = collection.id WHERE collection.organization_id = $2 AND ($3::int IS NULL OR collection.id = $3) AND ($4::text IS NULL OR collection.external_id = $4) AND ($5::text IS NULL OR collection.slug = $5) ORDER BY collection.updated_at DESC",
    ))
}
...
impl SelectCollectionsStmt {
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::StringSql, T2: crate::StringSql>(
        ...
        slug: &'a Option<T2>,
    ) -> CollectionRowQuery<'c, 'a, 's, C, CollectionRow, 5> {
        CollectionRowQuery {
            client,
...
pub fn select_collection_summaries() -> SelectCollectionSummariesStmt {
    SelectCollectionSummariesStmt(crate::client::async_::Stmt::new(
        "SELECT collection.*, coalesce(stats.num_sizes, 0) AS num_sizes, coalesce(stats.num_colors, 0) AS num_colors, coalesce(stats.num_styles, 0) AS num_styles, coalesce(pricing.json_data, '[]'::json) AS pricing FROM collection INNER JOIN ( SELECT group_collection.collection_id FROM group_collection INNER JOIN group_user ON group_user.group_id = group_collection.group_id WHERE group_user.user_id = $1 GROUP BY group_collection.collection_id UNION SELECT collection.id AS collection_id FROM collection WHERE collection.created_by = $1 ) AS requester_collections ON requester_collections.collection_id = collection.id LEFT JOIN ( SELECT size_collection.collection_id, count(DISTINCT size.id) AS num_sizes, count(DISTINCT color.id) AS num_colors, count(DISTINCT color.style_id) AS num_styles FROM color INNER JOIN size ON size.color_id = color.id INNER JOIN size_collection ON size.id = size_collection.size_id GROUP BY size_collection.collection_id ) AS stats ON stats.collection_id = collection.id LEFT JOIN LATERAL ( SELECT collection_pricelist.collection_id, json_agg( json_build_object( 'list', json_build_object( 'id', pricelist.id, 'external_id', pricelist.external_id, 'slug', pricelist.slug, 'name', pricelist.name ), 'date', collection_pricelist.price_date ) ) FILTER (WHERE collection_pricelist.collection_id IS NOT NULL) AS json_data, min(collection_pricelist.price_date) AS min_price_date FROM collection_pricelist INNER JOIN pricelist ON pricelist.id = collection_pricelist.pricelist_id WHERE collection_pricelist.collection_id = collection.id GROUP BY collection_pricelist.collection_id ) AS pricing ON pricing.collection_id = collection.id WHERE collection.organization_id = $2 AND ($3::int IS NULL OR collection.id = $3) AND ($4::text IS NULL OR collection.external_id = $4) AND ($5::text IS NULL OR collection.slug = $5) ORDER BY pricing.min_price_date DESC, collection.name ASC",
    ))
}
...
impl SelectCollectionSummariesStmt {
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::StringSql, T2: crate::StringSql>(
        ...
        external_id: &'a Option<T1>,
        slug: &'a Option<T2>,
    ) -> CollectionSummaryRowQuery<'c, 'a, 's, C, CollectionSummaryRow, 5> {
        CollectionSummaryRowQuery {
            client,
            params: [requester_id, organization_id, id, external_id, slug],
            stmt: &mut self.0,
...
pub fn get_collection_id() -> GetCollectionIdStmt {
    GetCollectionIdStmt(crate::client::async_::Stmt::new(
        "SELECT collection.id FROM collection WHERE collection.organization_id = $1 AND ($2::int IS NULL OR collection.id = $2) AND ($3::text IS NULL OR collection.external_id = $3) AND ($4::text IS NULL OR collection.slug = $4)",
    ))
}
...
pub fn insert_collection() -> InsertCollectionStmt {
    InsertCollectionStmt(crate::client::async_::Stmt::new(
        "INSERT INTO collection ( acronym, name, image_url, slug, external_id, organization_id, created_by) VALUES ( $1, $2, $3, $4, $5, $6, $7) RETURNING id",
    ))
}
...
pub fn update_collection() -> UpdateCollectionStmt {
    UpdateCollectionStmt(crate::client::async_::Stmt::new(
        "UPDATE collection SET acronym = coalesce($1, acronym), name = coalesce($2, name), image_url = coalesce($3, image_url), slug = coalesce($4, slug), external_id = coalesce($5, external_id) WHERE id = $6",
    ))
}
...
pub fn delete_collection() -> DeleteCollectionStmt {
    DeleteCollectionStmt(crate::client::async_::Stmt::new(
        "DELETE FROM collection WHERE organization_id = $1 AND id = $2",
    ))
}
...
pub fn associate_collection_sizes() -> AssociateCollectionSizesStmt {
    AssociateCollectionSizesStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM associate_collection_sizes($1, $2)",
    ))
}
...
pub fn replace_collection_pricelists() -> ReplaceCollectionPricelistsStmt {
    ReplaceCollectionPricelistsStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM replace_collection_pricelists($1, $2)",
    ))
}
...
pub fn set_new_collection_styles() -> SetNewCollectionStylesStmt {
    SetNewCollectionStylesStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM set_new_collection_styles($1, $2)",
    ))
}
...
pub fn set_new_collection_colors() -> SetNewCollectionColorsStmt {
    SetNewCollectionColorsStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM set_new_collection_colors($1, $2)",
    ))
}
