...
pub struct ColorRowQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    ...
    extractor: fn(&tokio_postgres::Row) -> Result<ColorRowBorrowed, tokio_postgres::Error>,
...
pub struct ColorRefsQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    ...
    params: [&'a (dyn postgres_types::ToSql + Sync); N],
    stmt: &'s mut crate::client::async_::Stmt,
    extractor: fn(&tokio_postgres::Row) -> Result<ColorRefsBorrowed, tokio_postgres::Error>,
...
impl ListColorsStmt {
    pub fn bind<'c, 'a, 's, C: GenericClient>(
        ...
        ColorRowQuery {
            ...
            stmt: &mut self.0,
            extractor:
                |row: &tokio_postgres::Row| -> Result<ColorRowBorrowed, tokio_postgres::Error> {
                    Ok(ColorRowBorrowed {
                        id: row.try_get(0)?,
                        organization_id: row.try_get(1)?,
                        slug: row.try_get(2)?,
                        external_id: row.try_get(3)?,
                        style_id: row.try_get(4)?,
...
impl GetColorRefsStmt {
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::StringSql, T2: crate::StringSql>(
        ...
        ColorRefsQuery {
            ...
            stmt: &mut self.0,
            extractor:
                |row: &tokio_postgres::Row| -> Result<ColorRefsBorrowed, tokio_postgres::Error> {
                    Ok(ColorRefsBorrowed {
                        id: row.try_get(0)?,
