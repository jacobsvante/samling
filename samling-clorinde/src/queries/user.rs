...
pub struct Veci32Query<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
    params: [&'a (dyn postgres_types::ToSql + Sync); N],
    stmt: &'s mut crate::client::async_::Stmt,
    extractor:
        fn(&tokio_postgres::Row) -> Result<crate::ArrayIterator<'_, i32>, tokio_postgres::Error>,
    mapper: fn(crate::ArrayIterator<'_, i32>) -> T,
}
impl<'c, 'a, 's, C, T: 'c, const N: usize> Veci32Query<'c, 'a, 's, C, T, N>
where
    C: GenericClient,
{
    pub fn map<R>(
...
pub fn select_users() -> SelectUsersStmt {
    SelectUsersStmt(crate::client::async_::Stmt::new(
        "WITH group_summaries AS ( SELECT \"group\".id, \"group\".slug, \"group\".external_id, \"group\".name, \"group\".description, coalesce(user_counts.n, 0) AS num_users, coalesce(collection_counts.n, 0) AS num_collections, coalesce(pricelist_counts.n, 0) AS num_price_lists FROM \"group\" LEFT JOIN LATERAL ( SELECT group_user.group_id, count(*) AS n FROM group_user WHERE group_user.group_id = \"group\".id GROUP BY group_user.group_id ) AS user_counts ON user_counts.group_id = \"group\".id LEFT JOIN LATERAL ( SELECT group_collection.group_id, count(*) AS n FROM group_collection WHERE group_collection.group_id = \"group\".id GROUP BY group_collection.group_id ) AS collection_counts ON collection_counts.group_id = \"group\".id LEFT JOIN LATERAL ( SELECT group_pricelist.group_id, count(*) AS n FROM group_pricelist WHERE group_pricelist.group_id = \"group\".id GROUP BY group_pricelist.group_id ) AS pricelist_counts ON pricelist_counts.group_id = \"group\".id WHERE ($1::int IS NULL OR \"group\".organization_id = $1) ) SELECT \"user\".*, coalesce(organizations.json_data, '[]'::jsonb) AS organizations, coalesce(\"groups\".json_data, '[]'::jsonb) AS \"groups\" FROM \"user\" LEFT JOIN ( SELECT user_organization.user_id, array_agg(organization.id) AS ids, jsonb_agg( jsonb_build_object( 'organization', jsonb_build_object( 'id', organization.id, 'name', organization.name, 'logo_url', organization.logo_url ), 'role_ids', user_organization.role_ids ) ) AS json_data FROM user_organization INNER JOIN organization ON organization.id = user_organization.organization_id GROUP BY user_organization.user_id ) AS organizations ON organizations.user_id = \"user\".id LEFT JOIN ( SELECT group_user.user_id, array_agg(group_user.group_id) AS group_ids, jsonb_agg(group_summaries.*) AS json_data FROM group_user INNER JOIN group_summaries ON group_summaries.id = group_user.group_id GROUP BY group_user.user_id ) AS \"groups\" ON \"groups\".user_id = \"user\".id WHERE ($1::int IS NULL OR $1 = any(organizations.ids)) AND ($2::int IS NULL OR \"user\".id = $2) AND ($3::text IS NULL OR lower(\"user\".email) = lower($3)) AND ( $4::int[] IS NULL OR jsonb_path_query_first( organizations.json_data, '$[*] ? (@.organization.id == $organization_id).role_ids', jsonb_build_object('organization_id', $1) ) @> to_jsonb($4) ) AND ($5::int[] IS NULL OR \"groups\".group_ids @> $5)",
    ))
}
            ...
            extractor:
                |row: &tokio_postgres::Row| -> Result<UserRowBorrowed, tokio_postgres::Error> {
                    Ok(UserRowBorrowed {
                        ...
                        profile_image: row.try_get(6)?,
                        last_sign_in: row.try_get(7)?,
                        organizations: row.try_get(8)?,
                        groups: row.try_get(9)?,
                    })
                },
...
pub fn get_org_user_id() -> GetOrgUserIdStmt {
    GetOrgUserIdStmt(crate::client::async_::Stmt::new(
        "SELECT \"user\".id FROM \"user\" INNER JOIN user_organization ON user_organization.user_id = \"user\".id WHERE user_organization.organization_id = $1 AND \"user\".id = $2",
    ))
}
...
pub fn get_role_ids() -> GetRoleIdsStmt {
    GetRoleIdsStmt(crate::client::async_::Stmt::new(
        "SELECT user_organization.role_ids FROM user_organization WHERE user_organization.user_id = $1 AND user_organization.organization_id = $2",
    ))
}
...
pub fn insert_user() -> InsertUserStmt {
    InsertUserStmt(crate::client::async_::Stmt::new(
        "INSERT INTO \"user\" ( name, email, password_hash, profile_image) VALUES ( $1, $2, $3, $4) RETURNING id",
    ))
}
...
pub fn update_user() -> UpdateUserStmt {
    UpdateUserStmt(crate::client::async_::Stmt::new(
        "UPDATE \"user\" SET name = coalesce($1, name), email = coalesce($2, email), password_hash = coalesce($3, password_hash), profile_image = coalesce($4, profile_image) WHERE id = $5",
    ))
}
...
pub fn upsert_user_organization() -> UpsertUserOrganizationStmt {
    UpsertUserOrganizationStmt(crate::client::async_::Stmt::new(
        "INSERT INTO user_organization ( user_id, organization_id, role_ids ) VALUES ( $1, $2, $3 ) ON CONFLICT ON CONSTRAINT user_organization_uq DO UPDATE SET role_ids = coalesce(excluded.role_ids, user_organization.role_ids)",
    ))
}
...
pub fn get_user_password_hash() -> GetUserPasswordHashStmt {
    GetUserPasswordHashStmt(crate::client::async_::Stmt::new(
        "SELECT \"user\".password_hash FROM \"user\" WHERE \"user\".id = $1",
    ))
}
...
pub fn replace_user_groups() -> ReplaceUserGroupsStmt {
    ReplaceUserGroupsStmt(crate::client::async_::Stmt::new(
        "SELECT * FROM replace_user_groups($1, $2)",
    ))
}
